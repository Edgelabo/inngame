...
try:
    ...
    class cv:
        COLOR_RGB2BGR = 4
        @staticmethod
        def cvt_color(image, code):  # pylint: disable=unused-argument
            return image
    
    class YoloxONNX:
        def __init__(self, **kwargs):
            # Vercel環境ではモデルファイルが利用できないため、スタブとして実装
            pass
...
class WebNinjutsuDetector:
    ...
    def __init__(self):
        # モデル設定
        self.model_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'model/yolox/yolox_nano.onnx')
    ...
    def _load_jutsu(self) -> List[List[str]]:
        """忍術ファイルを読み込み"""
        # Vercel環境用のハードコードされた忍術データ
        return [
            ['火遁', 'Fire Style', '豪火球の術', 'Fireball Jutsu', '巳', '寅', '申', '亥', '午', '寅'],
            ['', '', '分身の術', 'Clone Jutsu', '未', '巳', '寅'],
            ['', '', '口寄せの術', 'Summoning Jutsu', '戌', '亥', '酉', '申', '未'],
            ['火遁', 'Fire Style', '鳳仙花の術', 'Phoenix Flower Jutsu', '子', '寅', '戌', '丑', '卯', '寅'],
            ['水遁', 'Water Style', '水乱破の術', 'Water Trumpet', '辰', '寅', '卯']
        ]
    ...
    def process_image(self, image_data: str) -> Dict:
        ...
        try:
            ...
            frame = cv.cvt_color(np.array(image), cv.COLOR_RGB2BGR)
...
<body>
    <div class="container">
        ...
        <div class="demo-note">
            <strong>🎯 デモ注意事項</strong><br>
            このはVercelでホストされているデモ版です。AIモデルのファイルサイズの関係で、実際の印検出機能は制限されています。<br>
            完全な機能を体験するには、ローカル環境でお試しください。
        </div>
        ...
        <div class="demo-note">
            <strong>📋 検出可能な印</strong><br>
            子、丑、寅、卯、辰、巳、午、未、申、酉、戌、亥、壬、合掌
        </div>
    </div>
    ...
    <script>
        ...
        async function startCamera() {
            try {
                ...
                document.getElementById('status').textContent = 'カメラが開始されました（デモモード）';
                hideError();
        ...
        function connectWebSocket() {
            ...
            ws.onerror = function(error) {
                showError('WebSocket エラー（デモモードでは一部機能が制限されています）');
                // デモモードでのシミュレーション
                simulateDemo();
            };
        }
        ...
        function clearHistory() {
            ...
            // デモモード用のリセット
            document.getElementById('signHistory').textContent = '-';
            document.getElementById('jutsuDisplay').textContent = '術を検出中...';
        }
